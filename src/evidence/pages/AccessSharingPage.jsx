import { Check, Copy, Mail, RefreshCw, Send, ShieldCheck, UserPlus, UserX, XCircle } from 'lucide-react';
...
export default function AccessSharingPage() {
  ...
  async function copyInvitationLink(invitation) {
    ...
    if (!inviteUrl) {
      setState((current) => ({
        ...current,
        notice: 'Invitation link is not available yet. Refresh invitations or resend the invite to generate the manual link.',
      }));
      return;
    }
    try {
      ...
      setState((current) => ({
        ...current,
        notice: `Invitation link copied for ${invitation.invited_email}.`,
        copiedInviteId: invitation.invitation_id,
      }));
      window.setTimeout(() => {
        setState((current) => (
          current.copiedInviteId === invitation.invitation_id
      ...
      setState((current) => ({
        ...current,
        notice: 'Could not copy automatically. Select and copy the visible invitation link.',
      }));
    }
  }
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-[#101820]">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold text-gray-950 dark:text-white">{t('Invitations')}</h2>
            </div>
            <div className="space-y-3">
              {state.invitations.map((invitation) => (
                <article key={invitation.invitation_id} className="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        ...
                        {invitation.invite_url ? (
                          <span className="break-all rounded-md border border-gray-200 bg-gray-50 px-2 py-1 font-mono text-xs text-gray-600 dark:border-gray-800 dark:bg-black/20 dark:text-gray-300">
                            {invitation.invite_url}
                          </span>
                        ) : (
                          <span className="text-xs text-gray-500 dark:text-gray-400">{t('Manual invite link not available yet.')}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex shrink-0 flex-wrap gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-md border border-emerald-300 px-3 py-2 text-sm font-semibold text-emerald-800 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-emerald-900/70 dark:text-emerald-100 dark:hover:bg-emerald-950/30"
                      >
                        {state.copiedInviteId === invitation.invitation_id ? <Check size={15} aria-hidden="true" /> : <Copy size={15} aria-hidden="true" />}
                      ...
                      <button
                        ...
                        disabled={state.saving || invitation.status !== 'pending' || !invitation.can_cancel}
                  ...
                  {invitation.status === 'pending' && !invitation.can_cancel ? (
