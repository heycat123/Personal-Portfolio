...
const FILTER_LABELS = {
  canonical_name: 'Person/contact contains',
  entity_type: 'Type',
  review_status: 'Review state',
  promotion_state: 'Promotion',
  review_priority: 'Priority >=',
  confidence: 'Confidence >=',
  alias_count: 'Names used in documents >=',
  mention_count: 'Mentions >=',
  role_count: 'Roles >=',
  confirmed_alias_count: 'Confirmed >=',
...
const REVIEW_STATUS_OPTIONS = [
  ...
  { value: 'confirmed', label: 'Confirmed important' },
  { value: 'candidate', label: 'Reviewed' },
  { value: 'topic_only', label: 'Topic only' },
  { value: 'suppressed', label: 'Hidden from review' },
...
const PROMOTION_STATE_OPTIONS = [
  ...
  { value: 'confirmed', label: 'Confirmed important' },
  { value: 'candidate', label: 'Reviewed candidate' },
  { value: 'topic_only', label: 'Topic only' },
  { value: 'suppressed', label: 'Hidden from review' },
];
...
const ENTITY_COLUMNS = [
  {
    id: 'canonical_name',
    label: 'Person/contact record',
    help: 'Person, organization, or contact record found in the workspace. Filter searches names used in documents, alternate names, and record ids.',
    filterType: 'text',
  ...
  },
  {
    id: 'promotion_state',
    label: 'Promotion',
    help: 'Whether this record is confirmed, promoted for review, a candidate, topic-only, or suppressed.',
  ...
  },
  {
    id: 'confidence',
    label: 'Confidence',
    help: 'Average model/heuristic extraction confidence for this person/contact record. Filter means minimum confidence.',
    filterType: 'number',
    placeholder: '0.75',
    step: '0.01',
    max: '1',
  ...
  },
  {
    id: 'alias_count',
    label: 'Names used',
    help: 'Number of distinct aliases currently attached to this person/contact record by ingestion. Filter means minimum alias count.',
    filterType: 'number',
    placeholder: '2',
  ...
  },
  {
    id: 'mention_count',
    label: 'Mentions',
    help: 'Number of extracted mentions resolved to this person/contact record. Filter means minimum mention count.',
    filterType: 'number',
    placeholder: '10',
    headerClassName: 'min-w-[110px]',
  },
  {
    id: 'role_count',
    label: 'Roles',
    help: 'Number of role assertions attached to this person/contact record. Filter means minimum role count.',
    filterType: 'number',
    placeholder: '1',
  ...
  },
  {
    ...
    label: 'Confirmed',
    help: 'Number of human alias confirmations recorded for this person/contact record. Filter means minimum confirmation count.',
    filterType: 'number',
    placeholder: '1',
    headerClassName: 'min-w-[120px]',
  },
  {
    id: 'source_rows',
    label: 'Source rows',
    help: 'Number of source rows merged into this person/contact record. Filter means minimum source-row count.',
    filterType: 'number',
    placeholder: '1',
    headerClassName: 'min-w-[120px]',
  },
...
export default function EntitiesPage() {
    ...
    try {
      ...
      const result = await evidenceApi.updateEntity(
        ...
        entity.person_id,
        {
          ...
          reviewer_note: `Corrected person/contact record name from "${entity.canonical_name}" to "${canonicalName}".`,
        },
        { token },
      );
  ...
  const submitManualEntity = useCallback(async ({
    ...
    if (!canonicalName) {
      setState((current) => ({ ...current, actionError: new Error('Enter a person/contact record name first.') }));
      return;
    }
    ...
    try {
      ...
      recordFingerprint(result, 'Create person/contact record');
      ...
      if (personId && sourceContactLink?.contact_entity_link_id) {
        const attachResult = await evidenceApi.reviewContactEntityLink(
          ...
          sourceContactLink.contact_entity_link_id,
          {
            ...
            reviewer_note: `Created ${canonicalName} from contact ${contactPointLabel(sourceContactLink)} and confirmed the contact point to the new person/contact record.`,
          },
          { token },
        );
      ...
      const result = await evidenceApi.reviewContactEntityLink(
        ...
        link.contact_entity_link_id,
        {
          ...
          reviewer_note: decision === 'confirm'
            ...
            : decision === 'reject'
              ? `Rejected ${contactPointLabel(link)} for ${link.canonical_name || link.person_id || 'this person/contact record'}.`
      ...
      const result = await evidenceApi.bulkReviewContactEntityLinks(
        caseId,
        {
          ...
          reviewer_note: decision === 'reject'
            ? `Bulk rejected ${confirmableIds.length} contact link(s) from Contact link review.`
            : `Bulk confirmed ${confirmableIds.length} proposed contact link(s) from Contact link review.${skipped ? ` ${skipped} unresolved row(s) skipped.` : ''}`,
        },
        { token },
      );
  ...
  const renderReviewActionButtons = (detailEntity) => {
    const currentStatus = entityReviewStatus(detailEntity);
    return (
      <div className="grid gap-2 sm:grid-cols-2">
        {entityReviewActionOptions().map((option) => {
          ...
          <button
            key={option.status}
            type="button"
            onClick={() => updateEntityReviewStatus(detailEntity, option.status)}
    ...
    if (!aliases.length) {
      return (
        <p className="rounded-md border border-dashed border-gray-300 p-3 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">
          {t('No alternate names are currently attached to this person/contact record.')}
        </p>
      );
    }
        ...
        <div key={alias.normalized_alias || alias.alias} className="rounded-md border border-gray-200 p-3 text-sm dark:border-gray-800">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                {t('name for {name}; {count} occurrence(s)', { name: detailEntity.canonical_name, count: alias.occurrence_count || 0 })}
              </span>
            </div>
          ...
          {isVagueKinship && !isConfirmed ? (
            <p className="mt-2 rounded-md border border-amber-200 bg-amber-50 p-2 text-xs text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-100">
              {t('This family label is ambiguous. Confirm the relationship instead of making it a general name for this record.')}
            </p>
          ) : null}
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <button
              type="button"
              title={t('Confirm name "{alias}" should resolve to {name}.', { alias: alias.alias, name: detailEntity.canonical_name })}
            ...
            <button
              type="button"
              title={t('Reject name "{alias}" for {name}.', { alias: alias.alias, name: detailEntity.canonical_name })}
            ...
            <select
              ...
              title={t('Choose the correct person/contact record if this name is attached to the wrong person.')}
            >
            ...
            <button
              ...
              title={isVagueKinship ? t('Relationship words should be confirmed as relationships, not moved as general names.') : t('Reject this name on the current person/contact record and confirm it on the selected record.')}
            >
              {t('Move name')}
            </button>
          </div>
  ...
  const renderConfirmedAliasRows = (detailEntity) => {
    ...
    if (!aliases.length) {
      return (
        <p className="rounded-md border border-dashed border-gray-300 p-3 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">
          {t('No confirmed alternate names yet.')}
        </p>
      );
    }
    return aliases.map((alias) => (
      ...
      <div className="space-y-2">
        <div className="flex gap-1">
          <input
            ...
            placeholder={t('Search person/contact or name')}
        ...
        {activeSearch ? (
          <div className="text-xs text-gray-500 dark:text-gray-400">
            {targetLoading
              ...
              : t('{count} matching person/contact option(s)', { count: targetOptions.length })}
          </div>
        ) : null}
        <select
          ...
          title={t('Choose the person/contact record this contact point belongs to.')}
        >
          <option value="">{t('Choose person/contact')}</option>
        ...
        <div className="flex flex-wrap gap-1">
          ...
          >
            ...
            {t('Create person/contact record')}
          </button>
          <button
            type="button"
  ...
  const contactColumns = useMemo(() => [
    ...
    {
      id: 'canonical_name',
      header: t('Suggested person/contact'),
      help: t('Person/contact record the contact point currently points toward. Blank means the contact point is not yet tied to a person.'),
      render: (link) => (
        <div className="min-w-0">
          {link.person_id ? (
            <button
    ...
    },
    {
      id: 'confidence',
      header: t('Confidence'),
      help: t('Confidence of the suggested contact link.'),
    ...
    },
    {
      id: 'actions',
      header: t('Review'),
      help: t('Confirm when the contact point belongs to the selected person/contact; reject when it does not.'),
      filterable: false,
      sortable: false,
      ...
      return {
        ...
        render: (item) => (
          <div className="flex items-start gap-2">
            ...
            <Link to={`/evidence/cases/${caseId}/entities/${item.person_id}`} className="mt-0.5 shrink-0 text-gray-400 hover:text-sky-700 dark:hover:text-sky-300" title={t('Open person/contact record')}>
  ...
  const renderEntityDetailPanel = (detailEntity, { inDrawer = false } = {}) => (
    <aside className={inDrawer ? 'h-full overflow-auto overflow-x-hidden p-3 sm:p-4' : 'space-y-5'}>
      {detailEntity && inDrawer ? (
        <section className="sticky top-0 z-10 mb-3 rounded-lg border border-sky-200 bg-white p-3 shadow-sm dark:border-sky-900/70 dark:bg-[#101820]">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <div className="flex items-center gap-1 text-sm font-semibold text-gray-950 dark:text-white">
                {t('What should happen with this person/contact record?')}
                <InfoTip label={t('Choose the plain-language outcome. The system saves the technical status behind the scenes.')} />
              </div>
              <p className="mt-1 text-xs text-gray-600 dark:text-gray-400">
                {t('Current state')}: {t(humanizeKey(entityReviewStatus(detailEntity)))} - {t(humanizeKey(entityPromotionState(detailEntity)))}
              </p>
            </div>
            {renderReviewActionButtons(detailEntity)}
          </div>
        </section>
      ) : null}
      <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-[#101820]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h3 className="break-words text-base font-semibold text-gray-950 dark:text-white">{detailEntity?.canonical_name || t('Select a person/contact record')}</h3>
              {detailEntity ? (
                <button
                  type="button"
                  onClick={() => {
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{detailEntity ? truncateMiddle(detailEntity.person_id, 36) : t('Choose a row to inspect names used in documents and source details.')}</p>
          </div>
          {detailEntity ? (
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <StatusBadge status={reviewBadgeStatus(entityReviewStatus(detailEntity))} label={humanizeKey(entityReviewStatus(detailEntity))} />
              <StatusBadge status={promotionBadgeStatus(entityPromotionState(detailEntity))} label={humanizeKey(entityPromotionState(detailEntity))} />
            </div>
          ) : null}
        </div>
        ...
        {detailEntity ? (
          <div className="mt-4 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
            <div>
              <div className="flex items-center gap-1 text-xs font-semibold uppercase tracking-normal text-gray-500 dark:text-gray-400">{t('Confidence')} <InfoTip label={t('Average name/contact matching confidence from document processing.')} /></div>
            ...
            <div>
              <div className="flex items-center gap-1 text-xs font-semibold uppercase tracking-normal text-gray-500 dark:text-gray-400">{t('Confirmed')} <InfoTip label={t('Names confirmed by user for this person/contact record. The confirmation record includes who confirmed it.')} /></div>
        ...
        {detailEntity ? (
          <div className="mt-4 rounded-md border border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-800 dark:bg-black/20">
            ...
            <div className="mt-3">{renderReviewActionButtons(detailEntity)}</div>
          </div>
        ) : null}
        {detailEntity ? (
          <Link to={`/evidence/cases/${caseId}/entities/${detailEntity.person_id}`} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-sky-700 hover:text-sky-900 dark:text-sky-300 dark:hover:text-sky-100">
            ...
            {t('Open full person/contact record')}
          </Link>
        ) : null}
      </section>
      ...
      {detailEntity ? (
        <>
          <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-[#101820]">
            <div className="flex items-center justify-between gap-3">
              <h3 className="flex items-center gap-1 text-base font-semibold text-gray-950 dark:text-white">
                {t('Confirmed names')}
                <InfoTip label={t('Human-confirmed alternate names, nicknames, misspellings, or labels that should resolve to this person/contact record. The display name itself is not repeated here.')} />
              </h3>
              <button
                type="button"
              ...
              <div className="mt-4 rounded-md border border-sky-200 bg-sky-50 p-3 dark:border-sky-900/70 dark:bg-sky-950/30">
                <p className="text-xs text-sky-800 dark:text-sky-200">
                  {editingRelationship
                    ...
                    : t('Format: this person/contact has a relationship with another person/contact. Example: Maureen is maternal grandmother of Tiffany Curcio.')}
                </p>
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-[#101820]">
            <h3 className="text-base font-semibold text-gray-950 dark:text-white">{t('Names used in documents for {name}', { name: detailEntity.canonical_name })}</h3>
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-[#101820]">
            ...
            <ul className="mt-3 max-h-80 space-y-2 overflow-auto text-sm text-gray-700 dark:text-gray-300">
              ...
              {!(detailEntity.document_mentions || []).length ? (
                <li className="rounded-md border border-dashed border-gray-300 p-3 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">{t('No documents mention this person/contact yet.')}</li>
              ) : null}
            </ul>
          </section>
        </>
    ...
    <div>
      ...
      {roleResolutionReview ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <section className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-lg border border-gray-200 bg-white p-5 shadow-xl dark:border-gray-800 dark:bg-[#101820]">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-gray-950 dark:text-white">{t('Review relationship label')}</h2>
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  {t('The system found a possible existing person/contact reference in the relationship text. Confirm the suggested match or keep your original wording.')}
                </p>
              </div>
              <button
                type="button"
                ...
                <div key={`${item.input_role}-${item.normalized_role}`} className="rounded-lg border border-gray-200 p-3 dark:border-gray-800">
                  ...
                  <div className="mt-3 rounded-md bg-sky-50 p-3 text-sm text-sky-950 dark:bg-sky-950/40 dark:text-sky-100">
                    ...
                    {item.related_canonical_name ? (
                      <div className="mt-1 text-xs">{t('Related person/contact')}: {item.related_canonical_name}</div>
                    ) : null}
                  </div>
      ...
      {createEntityOpen ? (
        <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
          <button
            type="button"
            aria-label={t('Close create person/contact drawer')}
            ...
            <div className="flex items-start justify-between gap-3 border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-[#101820]">
              <div className="min-w-0">
                <div className="text-sm font-semibold text-gray-950 dark:text-white">
                  {createEntityContactLink ? t('Create Person/Contact From Contact') : t('New person/contact record')}
                </div>
                <div className="mt-1 break-words text-xs text-gray-500 dark:text-gray-400">
                  {createEntityContactLink
                    ...
                    : t('Add a person/contact record, names used in documents, and an optional relationship.')}
                </div>
              </div>
              <button
                type="button"
          ...
          <div className="mb-4 grid gap-3 text-sm text-gray-700 dark:text-gray-300 sm:grid-cols-2">
            ...
            <div className="rounded-md bg-gray-50 p-3 dark:bg-black/20">
              <div className="font-semibold text-gray-950 dark:text-white">{t('Names used in documents')}</div>
            ...
            <div className="rounded-md bg-gray-50 p-3 dark:bg-black/20">
              ...
              <p className="mt-1">{t('How this person/contact relates to another person/contact. Example: Leda Forseen is mother of Forest Lee.')}</p>
            </div>
          </div>
          {createEntityContactLink ? (
            <div className="mb-4 rounded-md border border-sky-200 bg-sky-50 p-3 text-sm text-sky-950 dark:border-sky-900/70 dark:bg-sky-950/30 dark:text-sky-100">
              ...
              <p className="mt-1 text-xs">
                {t('Create the person/contact record and optionally add a relationship below. After creation, this contact link will be confirmed for the new record.')}
              </p>
            </div>
          ) : null}
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="min-w-0 flex-1">
              <span className="text-xs font-semibold uppercase tracking-normal text-gray-500 dark:text-gray-400">{t('Names used in documents')}</span>
              <input
                value={createEntityForm.aliases}
                onChange={(event) => setCreateEntityForm((current) => ({ ...current, aliases: event.target.value }))}
                placeholder={t('Specific alternate names only, e.g. Leda Mae')}
                className="mt-2 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-950 dark:border-gray-700 dark:bg-[#0b1117] dark:text-gray-100"
              />
            </label>
          </div>
          {vagueKinshipAliases(parseCommaList(createEntityForm.aliases)).length ? (
            <div className="mt-3 rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-100">
              ...
              <p className="mt-1">
                {t('Words like mom, dad, grandma, brother, and aunt can mean different people in different conversations. Save them as a relationship below instead of a general name.')}
              </p>
            </div>
          ) : null}
          ...
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">
            {t('Use this when you know a real person or organization belongs in the case but the system did not create a clean person/contact record. Specific names are recorded as confirmed by user; relationship words are recorded as relationships so they do not collapse onto the wrong person.')}
          </p>
            </div>
          </aside>
        </div>
      ...
      <section className="mb-5 rounded-lg border border-sky-200 bg-sky-50 p-4 text-sm text-sky-950 dark:border-sky-900/60 dark:bg-sky-950/30 dark:text-sky-100">
        <div className="grid gap-3 lg:grid-cols-3">
          <div><strong>{t('Confirm here')}</strong> {t('records that a name, such as "grandma", should resolve to the selected person/contact record.')}</div>
          <div><strong>{t('Reject here')}</strong> {t('records that the name should not resolve to the selected person/contact record.')}</div>
          <div><strong>{t('Move name')}</strong> {t('rejects it on the current person/contact record and confirms it on the chosen correct record.')}</div>
        </div>
      </section>
      ...
      <div className="space-y-5">
        <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-[#101820]">
          <div className="mb-3 flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {t('Review Google Contacts phone and email links before relying on them. Confirmed links came from existing communication address mappings; ambiguous or unresolved links stay in review.')}
              </p>
            </div>
          ...
          <DataTable
            ...
            emptyTitle={state.contactLoading ? t('Loading contact links') : t('No contact links matched')}
            toolbar={(
              <div className="flex flex-col gap-2 rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-800 dark:bg-black/20 sm:flex-row sm:items-center sm:justify-between">
                <div className="text-gray-700 dark:text-gray-300">
                  ...
                  <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">{t('Bulk confirm only applies to rows that already have a suggested person/contact.')}</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
        ...
        <div>
          <DataTable
              ...
              return detail ? (
                <div>
                  <div className="mb-2 text-xs font-semibold uppercase tracking-normal text-gray-500 dark:text-gray-400">
                    {t('Names currently attached to {name}', { name: detail.canonical_name })}
                  </div>
                  <div className="grid gap-2 xl:grid-cols-2">{renderAliasRows(detail, true)}</div>
                </div>
              ) : (
                <div className="text-sm text-gray-600 dark:text-gray-400">{t('Loading names...')}</div>
              );
            }}
            mobileTitle={(item) => (
              <button
            ...
            mobileActions={(item) => (
              <Link to={`/evidence/cases/${caseId}/entities/${item.person_id}`} className="text-gray-400 hover:text-sky-700 dark:hover:text-sky-300" title={t('Open person/contact record')}>
                <ExternalLink size={15} aria-hidden="true" />
              </Link>
            )}
            pagination={{
                    ...
                    {people.length >= 2 ? (
                      <div className="mt-3 space-y-2">
                        <p className="text-xs text-gray-600 dark:text-gray-400">
                          {t('Choose the name that should stay. The other possible duplicate record will be folded into it, and names, mentions, contacts, and relationships will move to the kept record.')}
                        </p>
                        <div className="flex flex-wrap gap-2">
      ...
      {detailDrawerOpen ? (
        <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
          <button
            type="button"
            aria-label={t('Close person/contact details')}
          ...
          <div className="absolute bottom-0 right-0 top-0 flex w-screen max-w-full flex-col overflow-x-hidden border-l border-gray-200 bg-gray-50 shadow-2xl dark:border-gray-800 dark:bg-[#0b1117] sm:w-[92vw] sm:max-w-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-[#101820]">
              <div>
                <div className="text-sm font-semibold text-gray-950 dark:text-white">{t('Person/contact details')}</div>
                <div className="text-xs text-gray-500 dark:text-gray-400">{entity?.canonical_name || t('Select a person/contact record')}</div>
              </div>
              <button
                type="button"
                onClick={() => setDetailDrawerOpen(false)}
                        ...
                        <div key={address.address_id} className="rounded-md border border-gray-200 p-3 text-sm dark:border-gray-800">
                          ...
                          <div className="mt-1 text-xs text-gray-600 dark:text-gray-400">
                            {address.canonical_person_id
                              ...
                              : t('No person/contact record is linked yet.')}
                          </div>
                        </div>
                      ))}
