...
function createEvidenceProductionProxy(env) {
  ...
  if (!bearerToken) {
    console.warn(
      '[evidence-local-prod] No server-side bearer token found. ' +
      'In Cognito mode, sign in through the local app so browser requests carry Authorization. ' +
      'For token-file smoke mode, set EVIDENCE_LOCAL_API_BEARER_TOKEN or write .evidence-local/access-token.txt.',
    );
  } else {
